	...
	function getList(data){
		...
		for(let i=0; i<rows.length; i++){
			...
			<tr>
				...
				<td>${time}</td>
				<td>${row["term"]}</td>
				<td>${document.base.getDepartment(row["teacher"]["num"])}</td>
                <td>
                    <button class="btn btn-success btn-xs add"><i class="glyphicon glyphicon-plus"></i> Add </button>
